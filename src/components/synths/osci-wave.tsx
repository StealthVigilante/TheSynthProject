        ...
        <div className="grid grid-cols-4 gap-2 sm:grid-cols-8">
          {WAVEFORMS.map((w) => {
            ...
            return (
              <button
                key={w.id}
                onClick={() => setParam("oscillator.type", w.id)}
              ...
              >
                ...
                <span
                  ...
                  style={{ color: active ? T.accent : T.dim }}
                >
                  {w.label}
                </span>
              </button>
      ...
      <div className="px-6 mt-5 flex gap-4 flex-wrap">
        ...
        >
          ...
          <div className="flex gap-4">
            <Knob
              value={p("oscillator.spread")}
              min={0}
              max={100}
              step={1}
              label="Spread"
              unit="ct"
              onChange={(v) => setParam("oscillator.spread", v)}
              color={T.accent}
            ...
            />
            <Knob
              value={p("oscillator.count")}
              min={1}
              max={8}
              step={1}
              label="Voices"
              onChange={(v) => setParam("oscillator.count", Math.round(v))}
              color={T.accent2}
              trackColor={`${T.accent}20`}
              textColor={T.dim}
              size={58}
            ...
            {(["attack", "decay", "sustain", "release"] as const).map((stage) => {
              ...
              return (
                <Knob
                  ...
                  label={stage.slice(0, 3).toUpperCase()}
